...
namespace project_manage_system_backend.Services
{
    public class ProjectService : BaseService
    {
        ...
        //---編輯Project圖片---//
        public void EditProjectCoverImage(ProjectResultDto projectResultDto)
        {   
            //查詢對應的 Project
            var project = _dbContext.Projects.Find(projectResultDto.Id);
            //找不到對應的 Project
            if (project == null)
            {
                throw new Exception($"Project with ID {projectResultDto.Id} not found.");
            }
            //找到對應的 Project
            else
            {   
                //修改 CoverImage
                project.CoverImage = projectResultDto.CoverImage;

                //將修改寫入到資料庫
                if (_dbContext.SaveChanges() == 0)
                {
                    throw new Exception($"Edit Project: {projectResultDto.Id}  CoverImage fail!");
                }
            } 
        }

        /*舊的 GetProjectByUserAccount
        ...
        public List<ProjectResultDto> GetProjectByUserAccount(string account)
        {
            ...
            var query = (from up in user.Projects
                         select new ProjectResultDto { Id = up.Project.ID,
                                                       ...
                                                       CoverImage = up.Project.CoverImage }).ToList();
            return query;
        }

        //---從DB取得 所有 Projects by id---//
